...
import { ContainerAppHelper } from './ContainerAppHelper.js';
import { ContainerRegistryHelper } from './ContainerRegistryHelper.js';
...
export class AzureContainerApps {
  ...
  private static initializeHelpers() {
    ...
    let disableTelemetry =
      this.toolHelper.getInput('disableTelemetry').toLowerCase() === 'true';
  ...
  private static validateSupportedScenarioArguments() {
    ...
    if (
      !this.util.isNullOrEmpty(this.acrName) &&
  ...
  private static async setupResources() {
    ...
    if (!this.containerAppExists) {
      this.containerAppEnvironment =
        await this.getOrCreateContainerAppEnvironment(
          this.containerAppName,
          this.resourceGroup,
          this.location
        );
  ...
  private static getContainerAppName(): string {
    let containerAppName: string = this.toolHelper.getInput(
      'containerAppName',
      false
    );
  ...
  private static async getLocation(): Promise<string> {
    ...
    let resourceGroup: string = this.toolHelper.getInput(
      'resourceGroup',
      false
    );
    ...
    if (!this.util.isNullOrEmpty(resourceGroup)) {
      ...
      if (containerAppExists) {
        ...
        var environmentName =
          await this.appHelper.getExistingContainerAppEnvironmentName(
            this.containerAppName,
            resourceGroup
          );

        // Check if environment exists in the resource group provided and get the location
        var containerAppEnvironmentExistsInResourceGroup =
          !this.util.isNullOrEmpty(environmentName)
        ...
        if (containerAppEnvironmentExistsInResourceGroup) {
          ...
          location =
            await this.appHelper.getExistingContainerAppEnvironmentLocation(
              environmentName,
              resourceGroup
            );
          return location;
      ...
      let containerAppEnvironment: string = this.toolHelper.getInput(
  ...
  private static async getOrCreateContainerAppEnvironment(
    ...
    if (this.util.isNullOrEmpty(containerAppEnvironment)) {
      const existingContainerAppEnvironment: string =
        await this.appHelper.getExistingContainerAppEnvironment(resourceGroup);
    ...
    const containerAppEnvironmentExists: boolean =
      await this.appHelper.doesContainerAppEnvironmentExist(
        containerAppEnvironment,
        resourceGroup
      );
  ...
  private static async buildAndPushImageAsync() {
    ...
    let dockerfilePath: string = this.toolHelper.getInput(
      'dockerfilePath',
      false
    );
  ...
  private static async buildImageFromBuilderAsync(
    ...
    if (!this.util.isNullOrEmpty(runtimeStack)) {
      ...
      const platformName =
        runtimeStackSplit[0] == 'dotnetcore' ? 'dotnet' : runtimeStackSplit[0];
  ...
  private static setupContainerAppProperties() {
    ...
    const environmentVariables: string = this.toolHelper.getInput(
      'environmentVariables',
      false
    );
    const isCappUpdateCommandUsed: boolean =
      this.noIngressUpdate ||
    ...
    if (
      !this.util.isNullOrEmpty(this.toolHelper.getInput('containerName', false))
    ) {
      this.commandLineArgs.push(
        `--container-name ${this.toolHelper.getInput('containerName', false)}`
      );
    }
  }
