...
const ORYX_CLI_IMAGE: string =
  'mcr.microsoft.com/oryx/cli:builder-debian-bullseye-20230926.1';
const ORYX_BULLSEYE_BUILDER_IMAGE: string =
  'mcr.microsoft.com/oryx/builder:debian-bullseye-20240124.1';
const ORYX_BOOKWORM_BUILDER_IMAGE: string =
  'mcr.microsoft.com/oryx/builder:debian-bookworm-20240124.1';
const ORYX_BUILDER_IMAGES: string[] = [
  ORYX_BULLSEYE_BUILDER_IMAGE,
  ORYX_BOOKWORM_BUILDER_IMAGE,
];
const IS_WINDOWS_AGENT: boolean = os.platform() == 'win32';
const PACK_CMD: string = IS_WINDOWS_AGENT
...
export class ContainerAppHelper {
  ...
  public async createContainerApp(
    ...
    toolHelper.writeDebug(
      `Attempting to create Container App with name "${containerAppName}" in resource group "${resourceGroup}"`
    );
    try {
  ...
  public async createContainerAppFromYaml(
    ...
    toolHelper.writeDebug(
      `Attempting to create Container App with name "${containerAppName}" in resource group "${resourceGroup}" from provided YAML "${yamlConfigPath}"`
    );
    try {
  ...
  public async updateContainerApp(
    ...
    toolHelper.writeDebug(
      `Attempting to update Container App with name "${containerAppName}" in resource group "${resourceGroup}" `
    );
    try {
  ...
  public async updateContainerAppIngress(
    ...
    toolHelper.writeDebug(
      `Attempting to update Container App ingress with name "${containerAppName}" in resource group "${resourceGroup}"`
    );
    try {
  ...
  public async updateContainerAppFromYaml(
    ...
    toolHelper.writeDebug(
      `Attempting to update Container App with name "${containerAppName}" in resource group "${resourceGroup}" from provided YAML "${yamlConfigPath}"`
    );
    try {
   ...
   */
  public async doesContainerAppExist(
    containerAppName: string,
    resourceGroup: string
  ): Promise<boolean> {
    toolHelper.writeDebug(
      `Attempting to determine if Container App with name "${containerAppName}" exists in resource group "${resourceGroup}"`
    );
    try {
   ...
   */
  public async doesContainerAppEnvironmentExist(
    containerAppEnvironment: string,
    resourceGroup: string
  ): Promise<boolean> {
    toolHelper.writeDebug(
      `Attempting to determine if Container App Environment with name "${containerAppEnvironment}" exists in resource group "${resourceGroup}"`
    );
    try {
  ...
  public async doesResourceGroupExist(resourceGroup: string): Promise<boolean> {
    toolHelper.writeDebug(
      `Attempting to determine if resource group "${resourceGroup}" exists`
    );
    try {
  ...
  public async getDefaultContainerAppLocation(): Promise<string> {
    toolHelper.writeDebug(
      `Attempting to get the default location for the Container App service for the subscription.`
    );
    try {
  ...
  public async createResourceGroup(name: string, location: string) {
    toolHelper.writeDebug(
      `Attempting to create resource group "${name}" in location "${location}"`
    );
    try {
  ...
  public async getExistingContainerAppEnvironment(resourceGroup: string) {
    toolHelper.writeDebug(
      `Attempting to get the existing Container App Environment in resource group "${resourceGroup}"`
    );
    try {
   ...
   */
  public async createContainerAppEnvironment(
    name: string,
    resourceGroup: string,
    location?: string
  ) {
    const util = new Utility();
    toolHelper.writeDebug(
      `Attempting to create Container App Environment with name "${name}" in resource group "${resourceGroup}"`
    );
    try {
  ...
  public async disableContainerAppIngress(name: string, resourceGroup: string) {
    toolHelper.writeDebug(
      `Attempting to disable ingress for Container App with name "${name}" in resource group "${resourceGroup}"`
    );
    try {
   ...
   */
  public async updateContainerAppRegistryDetails(
    name: string,
    resourceGroup: string,
    registryUrl: string,
    ...
    toolHelper.writeDebug(
      `Attempting to set the Container Registry details for Container App with name "${name}" in resource group "${resourceGroup}"`
    );
    try {
  ...
  public async createRunnableAppImage(
    ...
    for (const builderImage of ORYX_BUILDER_IMAGES) {
      if (
        !util.isNullOrEmpty(builderStack) &&
      ...
      toolHelper.writeDebug(
        `Attempting to create a runnable application image with name "${imageToDeploy}" using the Oryx++ Builder "${builderImage}"`
      );
      ...
      try {
        ...
        toolHelper.writeWarning(
          `Unable to run 'pack build' command to produce runnable application image: ${err.message}`
        );
      }
    }
  ...
  public async createRunnableAppImageFromDockerfile(
    ...
    toolHelper.writeDebug(
      `Attempting to create a runnable application image from the provided/found Dockerfile "${dockerfilePath}" with image name "${imageToDeploy}"`
    );
    try {
      ...
      await util.execute(command);
      toolHelper.writeDebug(
        `Successfully created runnable application image from the provided/found Dockerfile "${dockerfilePath}" with image name "${imageToDeploy}"`
      );
    } catch (err) {
      toolHelper.writeError(err.message);
      throw err;
  ...
  public async determineRuntimeStackAsync(
    ...
    toolHelper.writeDebug(
      'Attempting to determine the runtime stack needed for the provided application source'
    );
    try {
      ...
      fs.unlink(oryxRuntimeTxtPath, (err) => {
        if (err) {
          toolHelper.writeWarning(
            `Unable to delete the temporary file "${oryxRuntimeTxtPath}". Error: ${err.message}`
          );
        }
      });
  ...
  public async setDefaultBuilder() {
    toolHelper.writeInfo(
      'Setting the Oryx++ Builder as the default builder via the pack CLI'
    );
    try {
  ...
  public async installPackCliAsync() {
    ...
    try {
      ...
      let commandLine = '';
      if (IS_WINDOWS_AGENT) {
        let packZipDownloadUri: string =
          'https://github.com/buildpacks/pack/releases/download/v0.31.0/pack-v0.31.0-windows.zip';
        let packZipDownloadFilePath: string = path.join(
          PACK_CMD,
          'pack-windows.zip'
        );
      ...
      toolHelper.writeError(
        `Unable to install the pack CLI. Error: ${err.message}`
      );
      throw err;
    }
  }
  ...
  public async enablePackCliExperimentalFeaturesAsync() {
    toolHelper.writeDebug(
      'Attempting to enable experimental features for the pack CLI'
    );
    try {
      ...
      toolHelper.writeError(
        `Unable to enable experimental features for the pack CLI: ${err.message}`
      );
      throw err;
    }
  }
